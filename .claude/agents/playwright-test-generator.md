...
- **Semantic locators first, in priority order**: verify every locator against the real live DOM before using it (via the browser tools, not a guess). `getByRole(role, { name: '...' })` is the primary choice. For form fields use `getByLabel(...)` or `getByPlaceholder(...)`. Use `getByText(...)` for unique non-interactive text. Use `getByTestId(...)` only as a last resort, when nothing semantic uniquely identifies the element. NEVER emit a raw CSS class selector (`page.locator('.class')`), structural XPath, or a bare tag/pseudo-role guess like `locator('generic')` lifted from an accessibility snapshot - those are snapshot role names, not real selectors, and will silently match nothing at test time.
...
- **Web-first assertions only, always awaited**: use `await expect(locator).toBeVisible()`, `.toHaveText()`, `.toContainText()`, `.toHaveValue()`, etc. so Playwright retries until the condition holds. NEVER extract a value first (`await locator.textContent()`) and assert on the plain value afterward (`expect(value).toBe(...)`) - that reads the DOM once, with no retry, and is a common source of race-condition flakiness. Every `expect(...)` call must be awaited.
- **No hardcoded dynamic numbers**: never assert on counts, totals, badges, or other numeric values that come from backend data unless THIS test's own setup steps guarantee that exact number. Specifically avoid `toHaveCount(<number>)` on dynamic lists/tables, `toHaveText('3')`/`toContainText('12')` on counters, cart badges, dashboards, or sums, and `expect(await locator.count()).toBe(N)` on backend-driven elements. Instead assert on: presence (`await expect(locator.first()).toBeVisible()`), non-empty (`expect(await locator.count()).toBeGreaterThan(0)`), relationships (compute the expected total from visible row values and compare), or format (a regex for currency/ID shapes, e.g. `/^\$[\d,]+\.\d{2}$/`).
- **Unique, run-scoped values for any field that must be unique**: for signup/add forms with a field that must be unique per submission (email, username, phone, tenant/org name, etc.), generate a fresh value at the top of the test using `Date.now()` or `crypto.randomUUID()`, store it once in a `const` object, and reuse those same variables for both the fill actions and any later assertions - never re-type the literal a second time. Respect the field's real format constraints (valid email TLD, correct phone length, alphanumeric username, letters/spaces-only name, password meeting the app's complexity rules). Example: `const id = Date.now(); const user = { email: \`test.${id}@example.com\`, name: \`Test User ${id}\`, phone: \`555${String(id).slice(-7)}\` };`. Never hardcode a value like `test@example.com` or `John Doe` into a field that must be unique. If a field genuinely cannot be unique (a seeded/fixed admin login), use the known credential as given - do not invent one.
- **Test isolation**: each test must validate one core behavior and not depend on mutable state left behind by another test. Generate unique test data (per the rule above) so the test does not conflict with itself or with other tests running in parallel.

# Assertion budget and efficiency (cost control - follow strictly)
Every tool call is a full, paid model turn, and every extra assertion is one more line that can break. A good test proves the ONE outcome the intent describes - nothing more.
- Write at most 5 `expect` assertions in the whole test; 2-4 is usually right. Assert only (1) the final outcome the test intent describes and (2) at most 1-2 key facts that prove it (e.g. the submitted values shown in the confirmation). Put the assertions for the final outcome LAST.
- Never assert an action's own immediate effect: no `toBeChecked` right after clicking a checkbox/radio, no `toHaveValue` right after `fill`, no `toBeVisible` on a button you are about to click - the action already fails loudly if it did not work.
- Never assert static page furniture: headings, labels, logos, menu items, link counts. If a page shows many headings or cards, assert at most the 1-2 that identify the page or result.
- Use a `browser_verify_*` tool only for an assertion you will actually keep.
- Fill a form with ONE `browser_fill_form` call covering all its text fields, instead of one call per field. Take a `browser_snapshot` once after arriving on a page (pass a small `depth`, 3-4) and only again when the page changes structurally - never after every action.
- Aim to finish in about 12 tool calls or fewer.
